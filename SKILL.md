...
| 🎬 推演 | scripts/prompts/simulate-scenarios.json | 模拟幸存方案 | ⛔**无论赌注高低必做，不受可逆小事门控影响**。**必须等方案评分后**，对每个幸存方案各起一个 agent(mcts-simulator)，**必须等所有推演 agent 全部返回并展示结果后**，再汇总💡发现。不允许只推演一个方案（指有多个方案时不能只做部分） |
