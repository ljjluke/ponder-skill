...
⛔ 每步必须按以下顺序完整执行，跳过任何一步即算流程失败。所有文件读取和脚本调用在后台完成，不展示给用户。禁止使用 bash echo 做等待或输出文本:

```
1. 前置: 查 top 3 历史 + 错误警告（后台执行）
2. 读 prompt: 读 scripts/prompts/<步骤>.json 的 prompt+schema（后台读取）
3. 加载引擎: 读 engine_ref 指向的 engine/*.md（后台读取）
4. 执行: 主线程直行；或起子 agent。agent 全部返回后，**必须先把每个 agent 的产出逐条展示到对话（用户能看到），再汇总对比**。禁止跳过展示直接汇总
