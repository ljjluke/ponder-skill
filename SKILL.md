...
**⛔ 内部文化概念思考，输出翻译为用户领域语言。**

框架使用中国古代文化概念(心斋、逍遥游、齐物、梦蝶、诸子百家等)作为"思维操作码"。
这些是内部思考的引擎，不是输出格式。

输出时必须将文化概念翻译为用户所在领域的对应表达:
...
| 朝菌之视 | 如果只有1天必须上线什么 | 如果下月必须盈利怎么调整 | 如果只有1次干预机会做什么 |
| 冥灵之视 | 这个架构10年后还成立吗 | 这个市场10年后的格局 | 这个治疗方案10年后的效果 |
| 列子御风 | 不干预用户会自然走向哪 | 不调整市场会自然走向哪 | 不治疗病情会自然走向哪 |
| 至人无己 | 不被自己的技术偏好影响 | 不被部门利益影响决策 | 不被科室立场影响方案 |
| 齐物 | 每个技术方案都有适用场景 | 每个策略都有其合理性 | 每个治疗方案都有适应证 |
| 梦蝶翻转 | 如果"不做"才是最优解 | 如果"放弃"才是最优 | 如果"不治疗"才是最优 |

**⛔ 禁止: 对非中文用户输出原词 / 不做翻译 / 用户问了才解释**
**✅ 正确: 内部想"鲲鹏之视"，输出说"从全局系统层面来看"**
