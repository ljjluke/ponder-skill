...
视角名称: [名称]
洞察: [核心发现]
数据来源: [具体数据/报告/日期] ← 必填，没有就搜
未验证假设: [这个视角依赖什么假设] ← 必填，没有就是猜
用户待确认: [什么需要用户确认] ← 有方向分支时必填
```

盲点A（缺数据）→ 自己去查，查到填数据来源
盲点B（缺方向）→ 问用户，问完用户才能继续
...
评分: [0-10]
评分依据: [具体数据支撑] ← 必填
不确定性: [什么情况下这个分数不成立] ← 必填
依赖的用户假设: [假设用户偏好什么，如不确定则标"待确认"] ← 必填
```
...
仍存疑: [还有什么不确定性] ← 必填
假设清单: [结论依赖的所有假设] ← 必填
用户确认: [用户是否已确认接受这些假设] ← 必填
```

"用户确认"为否 → 问用户 → 用户确认后结论才成立
