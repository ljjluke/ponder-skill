...
**Step 4（八卦镜 ×8）**: 8 个维度各自独立，每维度起一个 agent 评分。**每个 agent 返回后立即展示其详细评分和依据**（维度名、评分、证据、不确定性），不少于 3 行。全部收集完后用对比表格统一呈现。

**Step 5（方案 ×5-8）**: 每个 agent 生成一个方案方向，prompt 里告知"已有哪些方向"，避免重复。**每个 agent 返回后立即展示该方案的完整描述**（名称、依据、适用条件），不少于 3 行。全部收集后去重合并，用对比表格展示各方案差异。

**Step 6（收敛）**: 主线程 agent()，因为需要同时看所有方案做全局比较淘汰。展示时用对比表格，按领域语言描述强弱项。

**Step 7（推演 ×N）**: 从收敛拿到 survivors，每个调一次 agent("模拟方案: X..."，十天干框架)。**每个 agent 返回后立即展示该方案的推演详情**（各阶段评估、综合评分、风险点），不少于 5 行。展示时不出现十天干术语，只说"该方案在规划、执行、品质等阶段的评估结果"。全部收集后传给辩论。

**Step 8（辩论 ×N）**: 每方案一个 agent 构建立场（立论+反驳其他方案）。**每个 agent 返回后立即展示该方案的完整立场**（立论论据+对其他方案的质疑），不少于 3 行。全部收集后整合排名。
