...
## Phase 0.7: 天马行空 — External Anchor Divergence

**⛔ MANDATORY — 六视之前必须做。跳过=假发散。**

人脑的天马行空来自潜意识——洗澡、散步、睡觉时后台冒出的连接。LLM 没有潜意识, 它所有的"联想"都是前台的。所以"想一个疯狂的点子"没用——它找不到真正陌生的东西。

解决方式: 给一个**外部随机锚点**, 不是 LLM 自己选的。

### 第一步: 获取外部锚点

从以下来源随机取一条, 强制与当前问题建立关联:

```
选项A: 从用户最近对话中随机抽取一句无关的话
选项B: 随便打开一个网页/新闻标题/书籍段落
选项C: 从一个不相关的领域抽取一个核心概念
  (生物学/天文学/烹饪/音乐/体育/建筑...)

⛔ 锚点不能由 LLM 自己"想一个", 必须是外部给定的。
```
...
现在: 把这个锚点强行跟当前问题放在一起, 找出至少 1 个共通点。
规则:
- 不允许说"这没有关联"
- 允许牵强附会
- 越离谱越好
...
例子:
  问题: A股趋势分析
  锚点: 天气预报中的"台风路径预测"
  关联: "台风路径有多个可能方向, 市场走势也一样——不要追求精确预测, 要准备多个预案"
```

### 第三步: 带回问题

```
从第二步的牵强关联中, 提炼出 1 个可以用到当前问题的具体角度。
不需要保留锚点的全部——只需要保留那个"之前没想到"的东西。
```
