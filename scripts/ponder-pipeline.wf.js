...
// ═══ Step 3: Independent verification (checks for fake clarity) ═══
phase('独立验证')
const verify = await agent("独立审查\n\n结论: " + draftConclusion + "\n推理: " + draftReasoning +
  "\n\n检查以下内容：\n" +
  "1. 结论是否真的清晰？还是LLM假装清晰？\n" +
  "2. 推理链有没有断裂或跳过步骤？\n" +
  "3. 有没有未验证的假设被当作事实？\n" +
  "4. 有没有数据缺失的环节？\n" +
  "5. 如果有方向分支，LLM有没有自己替用户决定？\n\n" +
  "如发现"假装清晰" → verdict=REVISE，标注fake_clarity=true",
  { label: "独立验证",
    schema: { type: "object", properties: {
      verdict: { type: "string", enum: ["PASS", "REVISE"] },
      fake_clarity: { type: "boolean", description: "LLM是否假装结论清晰但实际上模糊" },
      issues: { type: "array", items: { type: "object", properties: {
        severity: { type: "string", enum: ["critical", "major", "minor"] },
        detail: { type: "string" },
        type: { type: "string", enum: ["fake_clarity", "missing_data", "skipped_step", "assumed_for_user", "other"] },
      }, required: ["severity", "detail", "type"] } },
