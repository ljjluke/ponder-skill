...
### Changed
- 输出规则重构: 每步输出推理过程(带步骤说明+推理链), 非一行标记
- 去领域化: SKILL.md示例全部用跨领域通用语言, 不绑定行业
- 神思/六视/八卦镜/反证分别列清, 发散子步骤完整展示
- 英文README补全缺失亮点: frame-breaking/reconsolidation/falsification

### Fixed
- 方案对比和评分依据恢复至进度标记中
- 输出过滤器改为"每行都是用户看得懂的洞察"

## 1.17.6 (2026-06-22)

### Changed
- 删除所有硬编码领域翻译表 — LLM不再受限3个领域, 无限域自适应
- 无用之用阶段重写 — 不是"想疯狂点子", 是"先让自己没用"

### Added
- bump_version.sh — 版本号替换脚本, 一次改4个文件

## 1.17.5 (2026-06-21)

### Added
- 需求打磨说明 (README) — 工具会帮你把需求想清楚, 不是拿到就做
- 天马行空阶段 — 心斋之后加入"放下→浮出→回头"三步骤
...
### Changed
- 流程从步骤改为管道(orchestrate.js + Workflow一体调用)
- 绝对规则改为违反=输出无效(不再显示中间步骤)
- 输出过滤器新增
...
### Added
- 十天干推演框架 — 甲→乙→丙→丁→戊→己→庚→辛→壬→癸
- 自进化引擎 evolve.js — 检测瓶颈→生成修复→上线规则
- MMA记忆引擎正式接入 — 自然语言存储, 语义匹配
- 知识保洁(decay/sleep/promote)

## 1.12.0 (2026-06-16)

### Added
- 预测编码引擎 — 线性步骤→递归预测-测试-纠正循环
- 双加工系统 — System 1(快速)/System 2(慢速)
- 用户画像 — 人物志九征→五行五常人格分类
- CLS记忆架构 — 模式分离/完成/多周期睡眠/突触稳态
- MCTS真实树结构 — 持久化存储
- 知识审计 — mma audit完整度/矛盾/过期/五行平衡
- 五行生克 — 相乘/相侮/逆生 + 跨知识promotes/inhibits
- 记忆再巩固主动内容更新

### Changed
- 核心逻辑英文化(SKILL.md中文1581→61仅文化术语)
- 输出改为亮点驱动 — 展示洞察不展示流程
- 三步用户访谈恢复(复述→探询→约束)
- 反猜测规则 — 所有结论必须有来源
- 流式输出 — 每步完成立即输出
...
### Fixed
- phase-enforce格式匹配修复
- memory-agent-guard格式匹配修复
- 决策点检测过窄导致引擎旁路
- alwaysApply: true → 安装后默认启用
