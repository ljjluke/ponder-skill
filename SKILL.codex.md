...
⛔ This is the Codex version. Use request_user_input tool for ALL user interaction. Ask ONE question at a time, each with 2-3 options. Do NOT use plain text Q&A — always use the request_user_input tool with option labels.

### Requirement Refinement
Use request_user_input one question at a time, covering: time/place/people/method/essence. Every question MUST have options.
...
📢 The following reasoning steps MUST be shown to user (process and results clearly presented for each step):
   ...
   3. **Need confirmation**: Among content expanded by Shensi/Divergence/Bagua, **only ask about matters involving user needs/preferences/real situations** (use request_user_input for user to pick). Pure analytical blindspots and perspectives go directly to solution phase, no need to ask.
```
...
### User Confirmation
Profile refresh at each step already covers most user confirmation. After synthesis, if there are still **core directional choices** requiring user's personal judgment (not information confirmation), use request_user_input with options. After user responds, output final conclusion. If nothing remains, directly output conclusion.
